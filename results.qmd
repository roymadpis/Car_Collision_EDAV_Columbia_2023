...
We first wish to Plot decomposition graph - to display the different components of the daily number of accidents time-series
...
```
<u>Comment :</u> We can see that crash accidents occur more during the work commute time and especially in the way back home between 15h to 17h.
...
```
##Is it more likely to Hit and Run in a specific Month / Week day / Hour?
>>>>>>> 902e0261d325027f70ed055ccd43516d304ff581
...
plot_hours <- ggplot(filtered_data) +
  ...
  labs(subtitle = 'Per Hour',
       x = "Crash Hour",
       y = NULL) +
  scale_fill_manual(values = c("No" = "#FF9900", "Yes" = "darkgreen")) +  # Customize colors if needed
  theme_minimal() +
  theme(legend.key.size = unit(0.5, "cm"))

# Arrange the two plots side by side
combined_plot <- plot_month + plot_weekday + plot_hours
...
# sf_data <- st_as_sf(dt_lane_freq_intersection, coords = c("Longitude", "Latitude"))
# 
# # Create a spatial index
# spatial_index <- st_as_sfc(sf_data)
# 
# # Apply the buffer to create a circular region around each point
# buffered_data <- st_buffer(sf_data, dist = 5000)  # Use a larger distance for a 5 km radius
# 
# # Check for overlaps with sf::st_intersects
# overlap <- st_intersects(buffered_data, spatial_index)
# 
# # Keep rows with no overlaps
# filtered_data <- sf_data[!overlap, ]
# 
# # Convert back to data frame if needed
# dt_lane_freq_filtered <- as.data.frame(filtered_data)
...
+ One key element that can be taken from that interactive map is that parking are place with high density of Hit/Run incidents. This correlate with a previous plot when we saw that in 'First_Harmful_Event' column, "Parked-Vehicle" is the category with the highest proportion of Hit/Run. 

  Moreover, it is visible that intersections are location with higher density of these accidents. Overall, this heat map help us visualize the previous pattern we have discovered earlier, it is clear that there is almost no Hit/Run incidents on highway like 15A. 
...
+ **Weather data**: we analyzed the given weather column and found there are more accidents on days with "good" weather condition. The problem is there is an internal asymmetry between the numer "bad" and "good" weather conditions. Bad consitions.


conditions does not necessarily imply a higher number of accidents. However to be sure about that we need to have the proportion of the number of days where the road conditions were good or not.
