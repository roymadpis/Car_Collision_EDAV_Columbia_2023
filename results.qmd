...
```{r}
holiday_data |> count(normalizeHolidayName)
```
...
ggplot(data_for_plot |> filter(holiday != "mean_month"), ) +
  geom_line(aes(group=holiday, x = Crash_week, y = Count_week, color = holiday))+
...
+ Moreover, we can see a decrease in the number of car accidents between 2020 - 2021, this can be explained by the covid-19 outbreak that led to massive lockdowns and reduced significantly the number of drivers on the roads. Since then, the average number of accidents is a bit lower than pre-covid.
...
# Assuming dt_temporal_day is your data frame

# Create a ggplot object
plot <- ggplot(dt_temporal_day, aes(x = Crash_Date, y = total_crash)) +
  geom_line(aes(group = 1)) +
  geom_point(aes(color = is_holiday)) +
  labs(title = 'Number of car crashes per weeks') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y")
  ...
  layout(updatemenus = list(
    list(
      ...
      buttons = list(
        ...
        list(label = "2018",
             method = "animate",
             args = list(list(frames = list(list(data = dt_temporal_day %>% filter(Year == 2018)))))),
        list(label = "2019",
             method = "animate",
             args = list(list(frames = list(list(data = dt_temporal_day %>% filter(Year == 2019)))))),
        list(label = "2020",
             method = "animate",
             args = list(list(frames = list(list(data = dt_temporal_day %>% filter(Year == 2020)))))
        )
      )
    )
  ))
