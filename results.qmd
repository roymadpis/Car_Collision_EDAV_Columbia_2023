...
result_df$holiday <- factor()


count(result_df, holiday)
...
ggplot(dt_temporal_week, ) +
  geom_line(aes(group=1, x = Crash_week, y = Count_week)) + #because we grouped earlier by Crash_week we need now to specify that we treat all the data as one group.
  geom_point(data = dt_temporal_week_only_holiday,aes(color = holiday,x = Crash_week, y = Count_week)) +
