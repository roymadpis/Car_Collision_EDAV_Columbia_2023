...
c |> filter(Month == "January" | Month == "February") |> write_csv("data/data_for_d3/Jan_data.csv")
...
+ We can observe that **most of the car accidents happen at an intersection**. Further, half of the crashes at these intersection have **traffic lights**. This highlights an important issue : traffic light are not sufficient to mitigate the risk of car accident. Despite not having the proportion of number of cars crossing intersection with traffic light vs the number of car crossing intersection without traffic light this indicates that traffic light are not efficient for reducing car accident. To improve the interpretation of these results it would be better if there was a data set giving us the proportion of intersection with traffic light and stop sign in the county. Without this information it is hard to tell if stop sign are better than traffic light to reduce the number of accidents. <u>NB:</u> In this study we focus only on the number of accidents and not on the fluidity of the traffic for instance. Needless to say that reducing cannot be the only factor to consider when designing traffic sign and the roads. 
