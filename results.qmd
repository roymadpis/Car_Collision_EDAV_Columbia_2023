...
```{r}
count(dt_responsible, ACRS_Report_Type)
```

## Who's at Fault ?
```{r}
# Create a data set of interest
dt_responsible <- data_crash[,c("Hit_Run","NonTraffic","At_Fault","Driver_Substance_Abuse","ACRS_Report_Type")]
...
mp1 <- vcd:: mosaic(Hit_Run ~  Driver_Substance_Abuse_A + Traffic + ACRS_Report_Type ,
 data = dt_responsible,
 direction = c("v", "v","h", "h" ))

mp2 <- vcd:: mosaic(Hit_Run ~  Driver_Substance_Abuse_B  + Traffic + ACRS_Report_Type,
 data = dt_responsible,
 direction = c("v", "v","h" ,"h"))
```
