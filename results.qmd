...
```
```{r}
# Check if the data function exists in the base package
exists("data", envir = baseenv(), mode = "function")

# Check if the data function exists in a specific package (e.g., dplyr)
exists("data", envir = asNamespace("dplyr"), mode = "function")
...
dt_temporal <- data[,c("Crash_Date_Time","Weather", "Hit_Run", "First_Harmful_Event",
                       "At_Fault")]
...
dt_lane <- data[,c("Road_Division", "Junction", "Road_Alignment" ,"Traffic_Control","Road_Name","Number_of_Lanes", "Hit/Run")]
dt_lane <- dt_lane %>%
rename(Hit_Run = "Hit/Run")
...
##Looking at the roads/ intersection (junction, traffic light and division of the road)
### Road Division
...
ggplot(dt_lane, aes(x=Traffic_Control))+
  geom_bar()
```
...
```
```{r}
count(dt_responsible, At_Fault)
dt_responsible <- dt_responsible %>%
  mutate(At_Fault = recode(At_Fault,
        "BOTH" = "Both",
        "DRIVER" = "Driver",
        "NONMOTORIST" = 'Non Motorist',
        "UNKNOWN"='Unknown')) 

dt_responsible1 <- dt_responsible[,c('At_Fault', 'Driver_Substance_Abuse')]

n_threshold = 10 
dt_responsible1 <- dt_responsible1 %>%
  group_by(At_Fault, Driver_Substance_Abuse) %>%
  mutate(Frequency = n()) %>%
  ungroup() %>%
  filter(Frequency >= n_threshold) %>%
  distinct(At_Fault, Driver_Substance_Abuse, .keep_all = TRUE)
...
##dt_responsible1$Frequency <- 1
ggplot(data = dt_responsible1,
       aes(axis1 = At_Fault, axis2 = Driver_Substance_Abuse, y = Frequency)) +
  geom_alluvium(aes(fill = At_Fault)) +
  geom_stratum() +
  geom_text(stat = "stratum",
            aes(label = after_stat(stratum))) +
  #scale_x_discrete(limits = c("Road Division", "Traffic Signal", "Junction type"),
...
There is nothing to exploit from this graph, there are too much information for driver and none for the other so we don't have more information.
