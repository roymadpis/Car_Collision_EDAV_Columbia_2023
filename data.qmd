...
First, we wish to explore are "Crash Date/Time" as a time-series to see if we can spot some trend or seasonality in the collision data. We wish to explore if there are more/less accidents in specific months/days of week/hours of day. Moreover, We wish to explore whether holiday days / weeks with holidays, tend to have more or less accidents.

Second, hit and run accidents is a problem which  

a phenomena that 
Similarly we want to use the geographical data (spatial data) to spot specific trends or interesting insights that are related to locations issues. 
Moreover, we will study the quality of the roads or the configuration of the roads (whether it is curved or not, whether it is a two-way road or a single). Further the weather conditions will reveal some important trend.
